...
//
//  SERVICE LINKS (Agnes 5, 2026-08-29): tenants can define profile.serviceLinks
//  as an array of { path, title, when } entries. When present, the LLM is
//  instructed to pick the topically-best-fit entry and drop exactly one
//  contextual link to it in the article body. Separate from post-to-post
//  interlinking (LINKED_PATHS) which is unchanged.
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ---- 1c) Build the APPROVED SERVICE LINKS block (Agnes 5, 2026-08-29) ----
    // Renders only when the tenant has serviceLinks configured. Instructs the
    // LLM to pick the best topical match and place exactly ONE contextual link
    // in the body. Separate from LINKED_PATHS (post-to-post interlinking).
